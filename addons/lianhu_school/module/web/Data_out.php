<?php
	...
	$top_action = array(
		array('action_name'=>'学校数据','action'=>'data_out'),
		array('action_name'=>'考试数据' ,'action'=>'data_out','arr'=>array('ac'=>'sroce') ),
	);	
	$page_name = '学校数据';
	if($ac=='sroce'){
		$page_name = '考试数据';
	...
	$class_grade 			= D('grade');
	$class_grade->each_page = 10000;
	$we7_js = 'no';

	if( $ac=='list' && $op=='list'  ) {
		$where["school_id"] = $this->school_info['school_id'];
		$re 	= $class_grade->getList($where);
		$grade  = $re['list'];
	}else{
		$grade  = $class_grade->getSchoolList();
	}
	if($ac=='list'&& $op=='class'){
		$gid 		 = $_GPC['gid'];
		$grade_name  = $this->gradeName($gid);
