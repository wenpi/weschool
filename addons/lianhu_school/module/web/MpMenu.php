<?php 
	...
	$sidebar_list = array(
		...
		array('fun_str'=>'mpMenu','fun_name'=>'公众号菜单设置'),
	);
	$top_action = array(
		array('action_name'=>'自定义菜单','action'=>'mpMenu' ),
		array('action_name'=>'历史菜单','action'=>'mpMenu','arr'=>array('status'=>'history') ),
    );

	$page_name    = '自定义菜单';
	
    $dos = array('display', 'save', 'remove', 'refresh', 'search_key', 'add', 'push', 'copy');
    $ac = in_array($ac, $dos) ? $ac : 'display';
    ...
    if($ac == 'display') {
        ...
        if(is_error($result)) {
            $this->myMessage($result['message'], '', 'error');
        }
    ...
    include $this->template('../admin/MpMenu');
    exit();
