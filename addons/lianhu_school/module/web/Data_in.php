...
if($ac=='teacher_data'){
	if($op=='new'){
		...
		if($in_arr){
			foreach ($in_arr as $key => $value) {
				if($teacher_data_type == 'teacher_tag'){
					$teacher_tag = $value['teacher_data'];
					$tags 		 = explode(',',$teacher_tag);
					$teacher_re  = D("teacher")->edit(array("teacher_id"=>$value['teacher_id']));
					foreach ($tags as $val) {
						$where["tag_name"] = $val;
						$where["school_id"]= getSchoolId();
						$result = D("teacherTag")->edit($where);
						if($result){
							$tag_id = $result['tag_id'];
						}else{
							$tag_id = D("teacherTag")->add($where);
						}
						$teacher_tags   = explode(',',$teacher_re['teacher_tags']);
						$teacher_tags[] = $tag_id;
					}
					$teacher_tags    = array_unique($teacher_tags);
					$teacher_tag_str = implode(',',$teacher_tags);
					$params['teacher_tags'] 	= $teacher_tag_str;
					$where_te['teacher_id']		= $value['teacher_id'];
					D("teacher")->edit($where_te,$params);
				}else{
					$params[$teacher_data_type] = $value['teacher_data'];
					$where['teacher_id'] 		= $value['teacher_id'];
					D("teacher")->edit($where,$params);
				}
				$success++;
			}
		}		
