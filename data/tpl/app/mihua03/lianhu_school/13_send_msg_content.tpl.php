<?php defined('IN_IA') or exit('Access Denied');?>            <?php  if(is_array($list)) { foreach($list as $row) { ?>
            ...
            <?php  $all_num               = count($student_ids);?>
                <div class="weui_media_box weui_media_text">
                    <h4 class="weui_media_title"><a href="<?php  echo $this->createMobileUrl('sendContent',array('type'=>'record','id'=>$row['record_id']))?>"><?php  echo date("m-d H:i",$row['add_time'])?>：<?php  echo $row['record_intro'];?></a></h4>
