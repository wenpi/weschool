...
<div>
    <ul class="ul-list focus-list" id="focus-list" style='padding-left:0'>
      <?php  if(is_array($list)) { foreach($list as $row) { ?>
         <li>
            <a href="<?php  echo $this->createMobileUrl('appointment_article',array('op'=>$row['appointment_id']));?>"><b><strong style="color:#ff0033;display:inline-block;float:left;">【<?php  if($row['status']==1) { ?>进行中<?php  } else { ?>已完成<?php  } ?>】</strong><?php  echo $row['appointment_name'];?></b>
              &nbsp;&nbsp;<?php  echo $this->clear_html_short($row['appointment_intro']);?>......</a>
            <p class="p-btm"><u><?php  if($row['appointment_type_limit']==0) { ?>全校<?php  } else if($row['appointment_type_limit']==1) { ?>年级<?php  } else if($row['appointment_type_limit']==2) { ?>班级<?php  } ?></u>|<time><?php  echo date("Y-m-d",$row['appointment_start']);?>--<?php  echo date("Y-m-d",$row['appointment_end']);?></time>|<i>
