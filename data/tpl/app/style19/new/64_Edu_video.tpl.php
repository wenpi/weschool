                    ...
                    <?php  if($row['list_src_type']==0) { ?>
                      <div class="w-embed w-video">
                        <video onclick="lookVideo(<?php  echo $row['list_id'];?>)" controls="" autoplay="0" webkit-playsinline="webkit-playsinline" x-webkit-airplay="allow" width="100%" height="100%" 
                              preload="auto" poster="<?php  echo $_W['attachurl'];?>/<?php  echo $row['list_img'];?>"  src="<?php  echo $row['list_src'];?>" >
                      </video> 
                      <div class="separator-bottom"></div>
                      </div>
                  <?php  } else if($row['list_src_type']==1) { ?>
                    <div class="w-embed w-video w-iframe"  onclick="lookIframe(this,<?php  echo $row['list_id'];?>,'<?php  echo $row['list_src_content']; ?>')" style="background-size:100%;padding-top: 56%;background-image:url(<?php  echo $this->imgFrom($row['list_img'])?>)">
