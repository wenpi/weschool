                        ...
                        <div class="col-md-12">
                            ...
                            <div class="portlet box green-turquoise">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="fa fa-navicon"></i><?php  echo $page;?>  </div>
                                        <div class="tools">
                                    ...
                                    <table class="table table-bordered table-striped table-condensed flip-content" >
                                        ...
                                        <tbody id='file_list'>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>每日都可点击</td>
                                                    <td>
                                                       <?php  if($database_much_num) { ?>
                                                            <span class="font-red">最新版本：<?php  echo $database_new_version;?>；可更新版本数：<?php  echo $database_much_num;?></span>
                                                        <?php  } else { ?>
                                                            <span class="font-blue">[无需更新,您的版本已是最新版：<?php  echo $database_new_version;?>]</span>
                                                        <?php  } ?>
                                                    </td>
                                                    <td>
                                                        <a href="<?php  echo $this->createWebUrl('systemfix',array('op'=>'fix_table'))?>"><i class="fa fa-cloud-download"></i>点击升级</a><span class="font-red">&nbsp;&nbsp;每次点击最多更新20条</span>
                                                    </td>
                                                </tr>
                                                <?php  if($_GPC['up']=='yes12as') { ?>
                                                <?php  if($count>100) { ?>
                                                <tr>
                                                    <td>文件升级【系统级】</td>
                                                    <td>每日都可点击[需先文件版本初始化] 
                                                        <a target="_blank" href="http://bbs.jiaxt.cn/thread-326.htm">（17/4/1）查看更新日志</a></td>
                                                    <td>
                                                        <?php  if($file_up_need) { ?>
                                                            <span class="font-red">需更新文件</span>
                                                        <?php  } else { ?>
                                                            <span class="font-blue">可能暂无更新</span>
                                                        <?php  } ?>
                                                    </td>
                                                    <td>
                                                        <a href="<?php  echo $this->createWebUrl('systemfix',array('op'=>'fix_file' ))?>"><i class="fa fa-cloud-download"></i>点击升级</a><span class="font-red">&nbsp;&nbsp;请先数据库升级</span>
                                                    </td>
                                                </tr>		
                                                <?php  } else { ?>
                                                <tr>
                                                    ...
                                                    <td></td>
                                                    <td>
                                                        <a href="<?php  echo $this->createWebUrl('systemfix',array('op'=>'init_file' ))?>">文件版本初始化【不会影响数据】</a>
                                                    </td>
                                                </tr>				
                                                <?php  } ?>
                                                <?php  } ?>
                                                   <?php  if(USE_MIDDLE ==1) { ?>
                                                        <tr>
                                                            <td>同步数据到中间库【公众号级别】</td>
                                                            <td>主要是旧数据</td>
                                                            <td></td>
                                                            <td>
                                                                <a href="<?php  echo $this->createWebUrl('dataToMiddle')?>">点击同步</a>
                                                            </td>
                                                        </tr>	
                                                    <?php  } ?>
                                                <tr>
                                                ...
                                                <tr class="font-red">
                                                    <td>最后一次访问云端信息【请核对】<br>登记域名非常重要如有修改立即联系</td>
                                                    <td>登记域名：<?php  echo $last_up_info['host_name'];?>;域名：<?php  echo $last_up_info['ask_url'];?></td>	
                                                    <td>IP：<?php  echo $last_up_info['ask_ip'];?> &nbsp;&nbsp;云端定时访问：<?php  echo $last_up_info['redis'];?>次</td>
                                                   <?php  $file_re = file_exists(MODULE_ROOT.'/myclass/src/uniacid.php');?>
                                                   <?php  if($file_re) { ?>
                                                        <td>使用的公号：<?php  echo D('uniacid')->getMpName($last_up_info['ask_uniacid']);?></td>				
                                                   <?php  } else { ?>
                                                        <td>使用的公号：<?php  echo $last_up_info['ask_uniacid'];?></td>				
                                                   <?php  } ?>
                                                </tr>	
                                                <!--更新考勤数据-->
                                                <?php  } else if($op=='update_school_grade_class_student') { ?>
                                                        <?php  if(is_array($class_list)) { foreach($class_list as $row) { ?>
                                                            <tr class='class_list' data-status='0' data-id= "<?php  echo $row['class_id'];?>" >
                                                                <td><?php  echo $this->classGradeName($row['class_id'])?> <i class="fa fa-toggle-right"></i><?php  echo $row['class_name'];?></td>
                                                                <td> 未更新</td>
                                                            </tr>
                                                        <?php  } } ?>													
                                                        <?php  if(is_array($building_list)) { foreach($building_list as $row) { ?>
                                                             <tr class='class_list' data-status='0' data-id= "building_id-<?php  echo $row['building_id'];?>" >
                                                                <td> 楼栋：<i class="fa fa-toggle-right"></i><?php  echo $row['building_num'];?></td>
                                                                <td> 未更新</td>
                                                            </tr>                                                       
                                                        <?php  } } ?>
                                                <?php  } ?>                                                                                
