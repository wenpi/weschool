...
<div class='container' style='padding:0 5px 10px;margin:0;width:100%'>
    <ul class="nav nav-tabs">
        <?php  if($isRoot == TRUE) { ?>
        <li <?php  if($op == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('project', array('op' => 'post'));?>">添加项目</a></li>
        <?php  } ?>
        <li <?php  if($op == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('project',array('op'=>'display'));?>">管理项目</a></li>
    </ul>
</div>
...
<div style="padding:15px;">
    <form id="form2" class="form-horizontal" method="post">
        ...
        <table class="table table-hover">
            ...
            <td><input type="checkbox" value="<?php  echo $item['projectid'];?>" name="delete[]"></td>
            <td><?php  echo $item['projectid'];?></td>
            <td><?php  echo $item['title'];?></td>
            <td><?php  echo $item['desc'];?></td>
            <td><?php  echo date("Y-m-d",$item['timecreate'])?></td>
            <td>
                <a href="<?php  echo $this->createWebUrl('user', array('op' => 'display', 'projectid' => $item['projectid']))?>"
                   title="管理用户" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-list"></span></a>

                <a href="<?php  echo $this->createWebUrl('table', array('op' => 'display', 'projectid' => $item['projectid']))?>"
                   title="管理数据" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-list"></span></a>

                <a href="<?php  echo $this->createWebUrl('rule', array('op' => 'display', 'projectid' => $item['projectid']))?>"
                   title="管理权限" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-list"></span></a>

                <a href="<?php  echo $this->createWebUrl('project', array('op' => 'post', 'id' => $item['projectid']))?>" title="编辑"
                   class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-edit"></span></a>

                <a onclick="return confirm('此操作不可恢复，确认吗？'); return false;"
                   href="<?php  echo $this->createWebUrl('project', array('id' => $item['projectid'],'op'=>'del'))?>" title="删除"
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        ...
                        <span class="help-block">对项目进行简单介绍</span>
                    </div>
                </div>
                <div class="form-group">
